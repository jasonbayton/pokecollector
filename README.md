...
| `TCGDEX_STANDBY_API_BASE` | A second catalogue, consulted by the scanner only when the first cannot be reached at all. A catalogue that answers, including one answering "no such card", is never second-guessed: two builds of the same data return results in a different order, which changes which candidate ranks first for a card whose name and number are shared. Leave unset for no standby. | *(empty)* |
