...
| `TCGDEX_API_BASE` | Card catalogue the app reads from. Point it at a self-hosted TCGdex server to stop depending on a single upstream host; the public catalogue is used if unset. Read at startup, so a change needs a restart. | `https://api.tcgdex.net/v2` |
| `TCGDEX_STANDBY_API_BASE` | A second catalogue, consulted only when the first cannot be reached at all. Used by the scanner and by the card lookups behind adding, detail, enrichment and price sync, so a card matched during an outage can also be added. A catalogue that answers, including one answering "no such card", is never second-guessed: two builds of the same data return results in a different order, which changes which candidate ranks first for a card whose name and number are shared. Leave unset for no standby. | *(empty)* |
| `TCGDEX_ASSETS_BASE` | Where card artwork is fetched from. Point it at a mirror of `assets.tcgdex.net`, such as a caching proxy on your own network, so cards keep their pictures when the CDN is unreachable. The mirror is tried first and the CDN second, so a mirror that is switched off or has never seen an image costs one failed request rather than a missing picture. Only the catalogue's own image URLs are rewritten; uploads and manually supplied images are untouched. A mirror served over plain HTTP is not used for the reference artwork behind visual verification, which requires HTTPS. | *(empty, the public CDN)* |
| `TCGDEX_ASSETS_MODE` | Whether the mirror above is asked before the CDN (`primary`) or only after it has failed (`standby`, also accepted as `fallback` or `secondary`). Both orders try both hosts, so this is a preference rather than a restriction. Mirror-primary suits a mirror on your own network: quicker, and an image it already holds needs no internet at all. Mirror-standby suits one kept purely for the days the CDN is down. A standby mirror is not used for visual verification's reference artwork, which picks a single URL with no second attempt. | `primary` |
