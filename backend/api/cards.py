...
def create_custom_card(
    ...
    # Annotated as BackgroundTasks so FastAPI still injects it; defaulted so a
    # direct caller can omit it. FastAPI rejects `BackgroundTasks | None` here,
    # because it tries to treat the union as a Pydantic field. This matches the
    ...
    card = Card(
        ...
        # image_url is the validated form, not data.image_url: upstream now
        # rejects non-public or non-HTTPS URLs before this point.
    ...
    # Serialisation is ownership-aware now, so the viewer has to be passed.
