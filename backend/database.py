...
def _run_migrations(conn):
    ...
    migrations = [
        ...
        "ALTER TABLE scan_job_items ADD COLUMN IF NOT EXISTS image_stored_at TIMESTAMP",
...
def migrate_custom_card_ownership():
    ...
    try:
        ...
        for source in legacy_cards:
            ...
            for model in (
                CollectionItem,
                # This fork keeps deleted rows in a recycle bin rather than
                # dropping them, so a card can be referenced by nothing but a
                # pending restore. Upstream has no such table and cannot know
                # to look here; leaving it out would let a restore recreate a
                # collection row pointing at another user's shared template.
                DeletedCollectionItem,
                WishlistItem,
                ProductCard,
                ProductLedgerEntry,
