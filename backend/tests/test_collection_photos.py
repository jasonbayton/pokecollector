...
class CollectionPhotoTests(unittest.TestCase):
    ...
    def test_photo_outlives_the_last_row_because_the_deletion_is_recoverable(self):
        """Deliberate divergence from upstream, retested rather than deleted.

        Upstream removes the owner's photo once the last collection row for a
        card goes, which is correct where a deletion is final. In this fork a
        deletion is archived to the recycle bin and can be restored, so
        destroying the photo at the same moment would hand back a card whose
        image had been thrown away behind the user's back. The photo is kept;
        Settings still offers "Delete my card photos" for clearing them on
        purpose.
        """
