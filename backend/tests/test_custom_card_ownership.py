...
class CustomCardOwnershipTests(unittest.TestCase):
    ...
    def test_match_notifications_use_the_manual_card_owners_telegram_settings(self):
        ...
        with patch(
            ...
            # Upstream mocks the id alone. This fork will not record a match on
            # an id that resolves and nothing else, because generated candidate
            # ids can resolve to a genuinely different card, so the catalogue
            # card has to corroborate the name and printed number first. The
            # notification routing under test is unaffected.
            return_value={"id": "base1-25", "name": "Owner-only match", "localId": "25"},
...
class CustomCardOwnershipMigrationTests(unittest.TestCase):
    def setUp(self):
        ...
        # Someone whose only claim on the card is a pending restore.
        self.binner = User(username="binner", hashed_password="x", role="trainer", is_active=True)
        self.db.add_all([self.first_admin, self.second_admin, self.trainer, self.binner])
        self.db.commit()
        for user in (self.first_admin, self.second_admin, self.trainer, self.binner):
            self.db.refresh(user)
        self.first_admin_id = self.first_admin.id
        self.second_admin_id = self.second_admin.id
        self.trainer_id = self.trainer.id
        self.binner_id = self.binner.id
    ...
    def test_legacy_card_goes_to_first_admin_and_other_users_receive_private_clones(self):
        ...
        self.assertEqual(
            ...
            [self.second_admin_id, self.trainer_id, self.binner_id],
        )
    ...
    def test_a_pending_restore_is_enough_to_earn_its_own_clone(self):
        """A card can be referenced by nothing but the recycle bin.

        This fork keeps deleted rows rather than dropping them, so a user whose
        only claim is a pending restore still has a claim. Upstream's migration
        does not know the table exists. Missing it left the bin row pointed at
        the card that becomes the first admin's shared template, so restoring
        would recreate a collection row on someone else's card.
        """
        self.db.close()
        with patch.object(database, "SessionLocal", self.Session):
            migrate_custom_card_ownership()
        ...
        self.db = self.Session()
        binned = self.db.query(DeletedCollectionItem).filter(
            DeletedCollectionItem.user_id == self.binner_id
        ).one()
        self.assertNotEqual(binned.card_id, "custom-legacy")

        clone = self.db.query(Card).filter(Card.id == binned.card_id).one()
        self.assertEqual(clone.custom_owner_id, self.binner_id)
        self.assertFalse(clone.is_shared_template)
        self.assertEqual(clone.custom_source_card_id, "custom-legacy")
